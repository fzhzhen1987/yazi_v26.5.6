--- @since 26.5.6
--- 基于官方 yazi-rs/plugins:git (26.5.6) 改造：
---   恢复 staged / unstaged 的区分（官方 26.x 不分），新增 CODES.staged
---   其余结构（root/bubble_up/propagate_down/add/remove/fetch/Linemode）原样取自官方 26.x
...
local CODES = {
	...
	untracked = 5,
	modified = 4,
	staged = 3.5, -- staged modification（自定义：已暂存的改动）
	added = 3,
	deleted = 2,
	updated = 1,
	clean = 0,
...
-- 按优先级排序：先用位置敏感的 pattern 区分 staged(第1列) / unstaged(第2列)
local PATTERNS = {
	{ "^!!", CODES.ignored },     -- !! = ignored
	{ "^%?%?", CODES.untracked }, -- ?? = untracked
	{ "^M ", CODES.staged },      -- M_ = staged modification
	{ "^A ", CODES.added },       -- A_ = staged addition
	{ "^D ", CODES.deleted },     -- D_ = staged deletion
	{ "^[MARC] ", CODES.staged }, -- R/C = renamed/copied (staged)
	{ " [MT]", CODES.modified },  -- _M/_T = unstaged modification
	{ " D", CODES.deleted },      -- _D = unstaged deletion
	{ "MM", CODES.modified },     -- MM = staged + unstaged (显示为 unstaged)
	{ "[MT]", CODES.modified },   -- fallback for M or T
	{ "[AC]", CODES.added },      -- fallback for A or C
	{ "U", CODES.updated },       -- U = unmerged
	{ "[AD][AD]", CODES.updated },
}
...
local function setup(st, opts)
	...
	local styles = {
		...
		[CODES.staged] = t.staged or ui.Style():fg("cyan"),
